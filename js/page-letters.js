...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // dbId → Supabase row id (for future delete/update)
  var letters = [];
  ...
  function loadLetters() {
    // Load sessionStorage first (instant display)
    try { letters = JSON.parse(sessionStorage.getItem('uah_letters') || '[]'); } catch(e) {}
    renderLetterList();

    // Then overlay with Supabase data if available
    if (u.email && typeof dbGetLetters === 'function') {
      dbGetLetters().then(function(res) {
        if (res.data && res.data.length) {
          letters = res.data.map(function(row) {
            return {
              dbId:      row.id,
              recipient: row.recipient,
              preview:   (row.content || '').slice(0, 55) + ((row.content || '').length > 55 ? '...' : ''),
              full:      row.content || '',
      ...
      }).catch(function(err) { console.warn('[letters] Supabase load failed:', err); });
    }
  }
  ...
  if (saveBtn) {
    saveBtn.addEventListener('click', function() {
      ...
      letters.unshift({
        ...
        preview:   text.slice(0, 55) + (text.length > 55 ? '...' : ''),
        full:      text,
        date:      dateStr
      });
      try { sessionStorage.setItem('uah_letters', JSON.stringify(letters)); } catch(e) {}
      ...
      // Persist to Supabase
      if (u.email && typeof dbSaveLetter === 'function') {
        dbSaveLetter(currentRecipient, text)
          .then(function(res) {
            // Attach the DB id to the local record for future use
            if (res.data && res.data.id) letters[0].dbId = res.data.id;
          })
          .catch(function(err) { console.warn('[letters] Supabase save failed:', err); });
      }
    });
  }
